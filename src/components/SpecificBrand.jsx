...
const SpecificBrand = () => {
  ...
  return (
    ...
    <div >
    <h2 className="text-center dark:text-white my-16 text-2xl md:text-4xl font-bold border-e-2">
        Trending
      ...
      {speciBrands.length !== 0 ?  (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 w-3/4 mx-auto gap-10">
          {speciBrands.map((speciBrand) => (<SpeciBrand key={speciBrand._id} speciBrand={speciBrand}></SpeciBrand>
          ))}

       </div>
      ):(
        <p className="text-center my-20 text-3xl text-red-600">No products available now for this brand.</p>
      )}
    </div>
      <Footer></Footer>
    </div>
